...
export default function ServiceConfigurator({ initialSelection, onUpdate }: ServiceConfiguratorProps) {
    ...
    useEffect(() => {
        console.log("🚀 ServiceConfigurator Fix v2.1 Loaded");

        if (!selection || !selection.base) {
            console.warn("ServiceConfigurator: Invalid selection state", selection);
            return;
        }
        ...
        try {
            ...
            const updatedDetails = selection.pricingDetails?.details || {
                ...
                densityTier: selection.bling?.density || 'None',
